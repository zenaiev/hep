...
parent = ROOT.Math.PxPyPzMVector(1.,1.,1.,1.)
def generate_and_decay(parent_mass, daughter_mass_1, daughter_mass_2, daughter1, daughter2):
  #return [ROOT.Math.PxPyPzMVector(1., 1., 1., 1.), ROOT.Math.PxPyPzMVector(1., 1., 1., 1.)]
  ...
  parent.SetPy(py)
  parent.SetPz(pz)
  parent.SetM(parent_mass)
  #parent = ROOT.Math.PxPyPzMVector(px, py, pz, parent_mass) 

  # Decay
  two_body_decay(parent, daughter_mass_1, daughter_mass_2, daughter1, daughter2)
  #return
  ...
  smear_momentum(daughter1)
  smear_momentum(daughter2)
...
def smear_momentum(particle):
  ...
  particle.SetPx(particle.Px() * fluct)
  particle.SetPy(particle.Py() * fluct)
  particle.SetPz(particle.Pz() * fluct)
...
def two_body_decay(parent, m1, m2, daughter1, daughter2):
  ...
  boost1 = boost_vector(daughter1)
  daughter1.SetPx(boost1.Px())
  daughter1.SetPy(boost1.Py())
  daughter1.SetPz(boost1.Pz())
  daughter1.SetM(boost1.M())
  boost2 = boost_vector(daughter2)
  daughter2.SetPx(boost2.Px())
  daughter2.SetPy(boost2.Py())
  daughter2.SetPz(boost2.Pz())
  daughter2.SetM(boost2.M())
...
if __name__ == '__main__':
  if 1:
    ...
    particle1 = ROOT.Math.PxPyPzMVector(1., 1., 1., 1.)
    particle2 = ROOT.Math.PxPyPzMVector(1., 1., 1., 1.)
    particle3 = ROOT.Math.PxPyPzMVector(1., 1., 1., 1.)
    particle4 = ROOT.Math.PxPyPzMVector(1., 1., 1., 1.)
    for i in tqdm.tqdm(range(nParticles)):
      tracks = []
      generate_and_decay(mass_k_zero, mass_pi_ch, mass_pi_ch, particle1, particle2)
      tracks += [particle1,particle2]
      generate_and_decay(mass_d_zero, mass_pi_ch, mass_pi_ch, particle3, particle4)
      tracks += [particle3, particle4]
      #assert len(tracks) == 4
